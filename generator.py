...
class PasswordGenerator:
    ...
    def __init__(self):
        ...
        self.label_frame = LabelFrame(
            self.window, text="Введите число символов")
        ...
        generate_btn = Button(
            self.button_frame, text="Генерировать пароль", command=self.generate_random_password)
        generate_btn.grid(row=0, column=0, padx=10)

        # Copy Password Button
        copy_btn = Button(self.button_frame,
                          text="Копировать пароль", command=self.copy_password)
